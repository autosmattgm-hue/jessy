...
const AI_TIMEOUT_MS = Number(process.env.JESSY_AI_TIMEOUT_MS || 4000);
...
module.exports = async function handler(req, res) {
  ...
  if (isRateLimited(clientIp(req))) {
    send(res, 429, { error: 'Jessy AI is getting a lot of requests. Please try again in a minute.' });
    return;
  }
  ...
  try {
    ...
    if (JSON.stringify(body).length > MAX_BODY_BYTES) {
      send(res, 413, { error: 'Message is too large.' });
      return;
    }

    const messages = normalizeMessages(body.messages, body.message);
    if (!messages.length || messages[messages.length - 1].role !== 'user') {
      send(res, 400, { error: 'Please send a message for Jessy AI.' });
      return;
    }
    ...
    if (!process.env.NVIDIA_API_KEY) {
      send(res, 200, { reply: createLocalBeautyReply(latestMessage), fallback: true });
      return;
    }
    ...
    if (latestMessage) {
      ...
      send(res, 500, { error: 'Jessy AI had a problem. Please try again.' });
    }
  }
};
